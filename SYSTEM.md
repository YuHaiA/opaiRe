...
- 修改文件：
  ...
  - `SYSTEM.md`
- 变更内容：
  - 提交邮箱阶段返回 `409` 时，注册流程改为返回 `retry_submit_email_409` 特殊状态。
  - 外层结果处理遇到该状态时，只按节流冲突计入重试，不记录域名失败，也不打印“域名注册失败，下一轮重新生成”。
  - 并发批次收集与单线程补货循环会把该状态视作可冷却重试类状态，避免错误进入成功、失败或切节点分支。
  - 新增回归测试覆盖提交邮箱 `409` 不触发域名失败记录或粘性域名清理。
- 修改原因：
  - 现场连续出现提交邮箱 `409` 后，系统仍将当前邮箱域名标记为注册失败，导致一批域名被反复重新生成，看起来像鉴权/指纹频繁异常。
  - `409` 在该阶段更像并发提交冲突或短时节流，不应污染域名失败统计。
- 影响范围：
  - 影响提交邮箱 `409` 的状态传播、域名失败统计与批次冷却。
  - 不改变 `403` 节点节流处理、无密码发信 `409` 硬错误计数、OAuth token 指纹兼容策略或成功入库逻辑。
...
- 修改文件：
  ...
  - `SYSTEM.md`
- 变更内容：
  - `提交邮箱环节异常, 返回: 409` 不再计入节点硬错误桶，也不会触发同批任务熔断或节点淘汰。
  - `无密码通道邮件发送异常, 返回: 409` 仍保留为可计数错误，达到阈值后继续按原逻辑熔断并切换节点。
  - 补充回归测试，覆盖提交邮箱 409 被忽略、无密码发信 409 仍可计数且成功后清零。
- 修改原因：
  - 现场日志显示同一批次内既有提交邮箱 409，又有算力挑战成功，说明提交邮箱 409 更像并发提交阶段的节流/冲突，不足以证明当前出口节点整体失效。
  - 将提交邮箱 409 作为硬淘汰信号会导致共享批次过早熔断、频繁切换节点，放大“服务器频繁”的体感。
- 影响范围：
  - 仅影响批次错误计数与节点淘汰触发条件。
  - 不改变注册主链路指纹、OAuth token 兼容指纹、邮箱提交请求本身、库存保存或 Clash 手动操作。
...
- 修改文件：
  ...
  - `SYSTEM.md`
- 变更内容：
  - Clash 节点测活失败触发淘汰时，仍会写入 `evicted_nodes` 并从 `tested_nodes` 活节点池、`preferred_nodes` 标优节点池同步移除。
  - 当有效候选数到达 `3` 个底线时，不再跳过拉黑；会先淘汰当前失败节点，然后触发节点池重建。
  - 底线重建会清空 `evicted_nodes`、`preferred_nodes`、`tested_nodes`，重新测活当前策略组全部节点，并把通过延迟测试的节点写回 `tested_nodes` 有效节点池。
  - `preferred_only_mode` 开关值仍会保留，但底线重建后标优池为空，需要后续真实生产成功再重新沉淀标优节点。
  - 补充回归测试，覆盖标优节点测活失败后清理标优池，以及到底线后清池并重新测活写回有效节点池。
- 修改原因：
  - 标优节点和底线节点都不应成为免死节点；如果测活失败，应先淘汰，再通过全策略组重测恢复可用候选池。
- 影响范围：
  - 影响 Clash 节点淘汰、底线重建与运行态节点池清理。
  - 不改变标优节点的生成规则、只用标优模式开关或原始代理池逻辑。
...
- 修改文件：
  ...
  - `SYSTEM.md`
- 变更内容：
  - 在“并发与系统”页新增“注册节奏”配置面板，可切换 `快速` / `保守` 预设，并直接编辑共享批次启动错峰倍率、无密码发信错峰倍率、空批缓冲秒数、403 冷却秒数、单线补货间隔秒数。
  - 前端 `fetchConfig` 现在会为旧配置自动补齐 `registration_timing` 默认值；保存前会校正数值范围，避免空值或非法值写入配置。
  - `快速` 预设对应后端默认加速参数；`保守` 预设恢复 v16 初版较慢但更稳的等待节奏。
- 修改原因：
  - 后端已将注册节奏改为可配置，但前端页面缺少入口，用户无法在面板中调整。
- 影响范围：
  - 仅影响配置页面展示、保存前校正与 `registration_timing` 配置写入。
  - 不改变任务执行核心逻辑、库存逻辑、邮箱逻辑或节点切换策略。
...
- 修改文件：
  ...
  - `SYSTEM.md`
- 变更内容：
  - 历史记录：Clash 节点淘汰前曾在有效节点只剩 `5` 个或更少时触发保底保护；当前逻辑已改为 `3` 个底线触发清池重建。
  - 保底保护优先按当前策略组的“测速通过节点池”计算；若该池为空，则回退按当前策略组过滤后的有效节点总数判断。
  - 节点测活失败但命中保底保护时，日志会明确显示“触发保底保护”，不再误报成普通剔除失败。
  - 历史记录：曾新增回归测试覆盖“候选池充足时正常拉黑”和“候选池只剩 5 个时跳过拉黑”两条路径；当前测试已随底线重建策略调整。
- 修改原因：
