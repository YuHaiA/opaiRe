...
- 修改文件：
  ...
  - `tests/test_sub2api_shared_switch_signal.py`
- 变更内容：
  - `Sub2API` 共享全局 Clash 节点模式下，补货流程不再“每一批都预切一次节点”；现在只会在本轮补货首批先切一次，后续批次默认复用当前节点。
  - 若本批次里真的出现 `switch_node` 故障信号，则不会立刻把整轮补货打成全局反复切节点，而是仅标记“下一批补货前刷新全局节点”。
  - 新增定向测试，覆盖共享全局节点模式下 `switch_node` 信号应被转成“下一批刷新请求”，而不是持续保留为强制切换。
- 修改原因：
  - 解决 `Sub2API` 补货在共享单节点模式下每批都打印 `[Sub2API补货] 切换全局节点...`、并频繁撞上 `10 秒冷却跳过`，造成“看起来一直在切节点”的实际问题。
- 影响范围：
  - 仅影响 `Sub2API` 补货模式在共享全局 Clash 节点下的切换时机；原始代理池、独立代理池与其他主流程逻辑不变。
...
- 修改文件：
  ...
  - `static/js/app.js`
- 变更内容：
  - `Sub2APIClient` 新增分页迭代能力，`get_all_accounts()` 改为复用分页迭代器收集，不再把“分页逻辑”只埋在一个整包接口里。
  - `Sub2API` 仓管主循环在 `auto_check=false` 时，已从“一次性拉全量列表再统计”改成“逐页拉取、逐页累计”的库存判断模式。
  - `Sub2API` 仓管与独立测活任务在 `auto_check=true` 时，已从“先拉完整个仓库，再一次性并发测活”改成“逐页拉取、逐页并发测活、逐页累计有效数”。
  - 云端库存接口对 `Sub2API` 单平台请求新增源头分页路径：后端会分批扫描并只返回当前请求页的数据，不再默认先把整个 `Sub2API` 仓库和所有 `usage` 一次性打满。
  - 云端库存前端改为按批次循环拉取每个平台分页数据，不再固定请求 `page_size=2000` 的大包；同时取消登录初始化时自动拉云端库存，改为只有进入“云端库存”页时才开始分批获取。
  - 云端库存列表预加载阶段不再为 `Sub2API` 每个账号补打一遍 `get_account_usage()`，避免一进页面就触发成百上千个 `/usage` 请求。
  - 批量刷新前构建 `Sub2API` 名称到 ID 映射时，也改为按页扫描，不再整仓一次性读入。
- 修改原因：
  - 解决 `Sub2API` 全量库存、全量测活、云端库存页预加载都走“大包一口气拉完”的高峰压力问题。
  - 减少“启动任务时看似卡住，实际上后台被云端库存页或全量巡检抢占”的链路干扰。
- 影响范围：
  - 影响 `Sub2API` 仓管补货判断、独立测活、云端库存页数据拉取与批量刷新映射构建。
  - 对外行为不变，但执行方式改为分批化，峰值请求压力会明显下降。

- 修改文件：
  - `routers/system_routes.py`
  - `static/js/app.js`
- 变更内容：
  - `/api/start` 现在会在真正拉起引擎前先打印启动预告，明确说明本次启动识别到的模式，以及接下来会先跑库存巡检、全量测活，还是直接进入常规量产。
  - 当模式为 `Sub2API` 且阈值大于 `0` 时，后端会明确提示“启动后会先执行全量库存 / 测活巡检”，避免用户在前几十秒只看到等待却不知道后台正在做什么。
  - 前端点击“启动”后不再先把协议模式直接标记为运行中，而是先写入一条“正在向后端发送启动请求”的即时系统日志，再等待后端确认成功后切换 `isRunning`。
  - 前端在 `CPA / Sub2API` 启动前会补充模式级提示，说明会先做仓管巡检或测活，减少“点击启动后短时间没注册日志”的误判。
- 修改原因：
  - 解决“点击启动后卡一段时间、日志没有立即提示当前在做什么”的可观测性问题。
  - 降低 `Sub2API` / `CPA` 这类重启动流程在前端看起来像假死的体感。
- 影响范围：
  - 仅增强启动阶段的状态提示与前端时序，不改变实际注册、补货、测活或节点切换逻辑。
  - 对 `Sub2API` 和 `CPA` 模式的体感改善最明显，常规量产模式也会获得更清晰的启动预告。
