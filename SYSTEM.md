...
- 修改文件：
  ...
  - `SYSTEM.md`
- 变更内容：
  - Clash 节点测活失败触发淘汰时，除了写入 `evicted_nodes` 并从 `tested_nodes` 活节点池移除，也会同步从 `preferred_nodes` 标优节点池移除。
  - `preferred_only_mode` 仍会保留，只是失败节点不再因为曾经标优而继续留在标优候选里。
  - 保底保护逻辑保持不变：当有效候选数已低于保底阈值时，仍会跳过拉黑，也不会清理标优池，避免节点池被抽干。
  - 补充回归测试，覆盖标优节点测活失败后同时从活节点池和标优池移除。
- 修改原因：
  - 标优节点代表历史实战产出，不代表永久健康；如果后续测活失败，应按普通节点一样拉黑并移出候选池。
- 影响范围：
  - 仅影响 Clash 节点淘汰后的运行态配置清理。
  - 不改变标优节点的生成规则、只用标优模式开关、低存量保底保护或原始代理池逻辑。
