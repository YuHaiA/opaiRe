...
def check_nginx_upstreams() -> CheckResult:
    ...
    want = f"server {SERVER4_EXPECTED_PRIVATE_IP}:{SERVER4_REALITY_PORT};"
...
def apply_nginx_fix() -> CheckResult:
    script = f"""
set -e
cp {NGINX_CONF} {NGINX_CONF}.bak-codex-reconcile-$(date +%Y%m%d-%H%M%S)
python3 - <<'PY'
from pathlib import Path
path = Path({NGINX_CONF!r})
text = path.read_text(encoding='utf-8')
text = text.replace('server 10.0.0.112:24444;', 'server 10.0.0.154:24444;')
...
def check_subscription_domain_mode() -> CheckResult:
    ...
    text = res.stdout or ""
    ok = (
        "name: server4-reality-443" in text
