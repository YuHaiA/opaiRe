...
createApp({
    ...
    methods: {
        ...
        async updateFromGitHub() {
            const confirmed = await this.customConfirm("确认从 GitHub 拉取当前分支最新代码并同步依赖吗？\n\n此操作不会自动重启服务。若拉到新代码，需要你手动重启项目。");
            if (!confirmed) return;
            this.isGitUpdating = true;
            this.gitUpdateResult = null;
            try {
                this.showToast("正在从 GitHub 拉取最新代码...", "info");
                const res = await this.authFetch('/api/system/update_from_github', {
                    method: 'POST',
                    body: JSON.stringify({ remote: 'origin' })
                });
                const data = await res.json();
                this.gitUpdateResult = data;
                if (data.status === 'success') {
                    this.showToast(data.message || 'GitHub 更新完成', 'success');
                } else if (data.status === 'warning') {
                    this.showToast(data.message || 'GitHub 更新未完全成功', 'warning');
                } else {
                    this.showToast(data.message || 'GitHub 更新失败', 'error');
                }
            } catch (e) {
                this.showToast("GitHub 更新请求失败", "error");
            } finally {
                this.isGitUpdating = false;
            }
        },
        formatTime(dateStr) {
            if (!dateStr) return '-';
