...
def fetch_subscription_text(url: str, proxies: Optional[dict] = None) -> SubscriptionFetchResult:
    ...
    best_status = 0
    last_error = ""
    routes = [("proxy", proxies)] if proxies else []
    routes.append(("direct", None))

    for route_name, route_proxies in routes:
        for impersonate, user_agent in _BROWSER_PROFILES:
            kwargs = {
                "headers": _build_headers(normalized_url, user_agent),
                "timeout": 30,
                "allow_redirects": True,
                "impersonate": impersonate,
            }
            if route_proxies:
                kwargs["proxies"] = route_proxies
            try:
                response = cffi_requests.get(normalized_url, **kwargs)
            except Exception as exc:
                last_error = f"{route_name}:{impersonate}: {exc}"
                continue
            ...
            last_error = _failure_message(status_code)

    if best_status:
        return SubscriptionFetchResult(ok=False, status_code=best_status, message=_failure_message(best_status))
