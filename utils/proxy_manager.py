...
from utils.clash_group_utils import merge_proxy_provider_metadata, resolve_group_name, resolve_switchable_group_name, is_switchable_group
...
def _is_real_docker_runtime() -> bool:
    if not os.path.exists('/.dockerenv'):
        return False
    try:
        with open('/proc/1/cgroup', 'r', encoding='utf-8', errors='ignore') as f:
            marker = f.read().lower()
        return any(token in marker for token in ('docker', 'containerd', 'kubepods'))
    except Exception:
        return False

_IS_IN_DOCKER = _is_real_docker_runtime()
_global_switch_lock = threading.Lock()
_raw_proxy_evict_lock = threading.Lock()
_last_switch_time = 0
...
DEFAULT_RAW_PROXY_EVICT_MAX_PER_WINDOW = 5
DEFAULT_RAW_PROXY_EVICT_WINDOW_SECONDS = 60.0
DEFAULT_RAW_PROXY_MIN_REMAINING = 1
_raw_proxy_evict_history = deque()
...
def _parse_int_setting(value, default: int, minimum: int = 0) -> int:
    try:
...
def _parse_float_setting(value, default: float, minimum: float = 0.0) -> float:
    try:
...
def _raw_proxy_evict_settings(raw_conf: dict) -> tuple[bool, int, float, int]:
    source = raw_conf if isinstance(raw_conf, dict) else {}
    enabled = _parse_bool_setting(source.get("auto_evict_failed", True), True)
    max_per_window = _parse_int_setting(
        source.get("auto_evict_max_per_window", DEFAULT_RAW_PROXY_EVICT_MAX_PER_WINDOW),
        DEFAULT_RAW_PROXY_EVICT_MAX_PER_WINDOW,
        minimum=1,
    )
    window_seconds = _parse_float_setting(
        source.get("auto_evict_window_seconds", DEFAULT_RAW_PROXY_EVICT_WINDOW_SECONDS),
        DEFAULT_RAW_PROXY_EVICT_WINDOW_SECONDS,
        minimum=1.0,
    )
    min_remaining = _parse_int_setting(
        source.get("auto_evict_min_remaining", DEFAULT_RAW_PROXY_MIN_REMAINING),
        DEFAULT_RAW_PROXY_MIN_REMAINING,
        minimum=0,
    )
    return enabled, max_per_window, window_seconds, min_remaining


def _prune_raw_proxy_evict_history(now: float, window_seconds: float) -> None:
    while _raw_proxy_evict_history and now - _raw_proxy_evict_history[0] >= window_seconds:
        _raw_proxy_evict_history.popleft()
...
def _is_real_proxy_node(proxies_data: dict, node_name: str) -> bool:
    name = str(node_name or "").strip()
    if not name:
        return False
    if name.upper() in {"DIRECT", "REJECT", "REJECT-DROP", "PASS"}:
        return False
    meta = proxies_data.get(name) if isinstance(proxies_data, dict) else None
    if meta is None:
        # proxy-provider 节点在部分 Mihomo 版本中不会展开到 /proxies 顶层。
        return True
...
def _hydrate_provider_proxy_metadata(current_api_url: str, headers: dict, proxies_data: dict) -> dict:
    try:
        response = std_requests.get(
            f"{current_api_url}/providers/proxies",
            headers=headers,
            timeout=5,
        )
        if response.status_code == 200:
            return merge_proxy_provider_metadata(proxies_data, response.json() or {})
    except Exception:
        pass
    return proxies_data


def _fetch_group_delay_map(current_api_url: str, headers: dict, group_name: str, timeout_ms: int = 6000) -> dict[str, int]:
    encoded_group = urllib.parse.quote(str(group_name or ""), safe="")
    response = std_requests.get(
        f"{current_api_url}/group/{encoded_group}/delay",
        headers=headers,
        params={"timeout": timeout_ms, "url": "https://www.gstatic.com/generate_204"},
        timeout=max(45.0, timeout_ms / 1000.0 + 30.0),
    )
    if response.status_code != 200:
        raise RuntimeError(f"group delay HTTP {response.status_code}: {response.text[:160]}")
...
def _resolve_group_candidate_nodes(proxies_data: dict, group_name: str, current_node: str = "", clash_conf: Optional[dict] = None):
    ...
    valid_nodes = [
        ...
        and _is_real_proxy_node(proxies_data, node)
...
def _resolve_effective_candidate_count(proxy_url, clash_conf: dict, current_node: str) -> Optional[int]:
    ...
    try:
        ...
        if resp.status_code == 200:
            proxies_data = resp.json().get("proxies", {})
            proxies_data = _hydrate_provider_proxy_metadata(current_api_url, headers, proxies_data)
...
def _probe_clash_group_nodes(proxy_url, group_name: str) -> tuple[list[str], str]:
    ...
    runtime_nodes = runtime_group.get("all")
    nodes = [
        node for node in runtime_nodes
        if _is_real_proxy_node(proxies_data, node)
    ] if isinstance(runtime_nodes, list) else []
    if not nodes:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if resolved_group:
                ...
                if not _is_real_proxy_node(proxies_data, current_node):
                    return False, f"当前选中的 Clash 节点 [{clean_for_log(current_node)}] 不是可标优的真实出站节点"
    except Exception:
        pass
...
def evict_current_proxy_or_node(proxy_url=None):
    ...
    if runtime_cfg.is_raw_proxy_pool_enabled():
        # 多线程注册可能同时发现不同坏代理。串行执行“读取最新配置 → 删除 → 热重载”，
        # 避免两个线程各自基于旧快照写回，导致其中一条坏代理被意外恢复。
        with _raw_proxy_evict_lock:
            config_data = _load_runtime_config_for_write()
            raw_conf = config_data.get("raw_proxy_pool", {})
            if not isinstance(raw_conf, dict):
                raw_conf = {}
            source_entries = raw_conf.get("proxy_list", [])
            success_entries = raw_conf.get("success_proxy_list", [])
            if not isinstance(source_entries, list):
                source_entries = []
            if not isinstance(success_entries, list):
                success_entries = []
            success_pool_enabled = _parse_bool_setting(raw_conf.get("success_pool_enabled", False), False)
            active_entries = success_entries if success_pool_enabled else source_entries
            active_pool_label = "成功代理池" if success_pool_enabled else "原始代理池"
            ...
            kept_success_entries = [
                entry for entry in success_entries
                if runtime_cfg.normalize_raw_proxy_entry(entry) != target
            ]
            active_normalized = runtime_cfg.normalize_raw_proxy_list(active_entries)
            kept_active_entries = kept_success_entries if success_pool_enabled else kept_source_entries
            kept_active_normalized = runtime_cfg.normalize_raw_proxy_list(kept_active_entries)
            if len(kept_active_normalized) == len(active_normalized):
                return False, f"{active_pool_label}里未找到目标代理: {get_display_name(normalized_proxy)}"

            auto_evict_enabled, max_per_window, window_seconds, min_remaining = _raw_proxy_evict_settings(raw_conf)
            if not auto_evict_enabled:
                return False, "SKIP_EVICT_GUARD: 活动代理池自动淘汰已关闭；本轮仍会自动切换下一条。"
            if len(kept_active_normalized) < min_remaining:
                return False, (
                    "SKIP_EVICT_GUARD: "
                    f"{active_pool_label}至少保留 {min_remaining} 条，当前剩余 {len(active_normalized)} 条；"
                    "已暂停持久删除，本轮仍会自动切换下一条。"
                )

            now = time.monotonic()
            _prune_raw_proxy_evict_history(now, window_seconds)
            if len(_raw_proxy_evict_history) >= max_per_window:
                retry_after = max(1, int(window_seconds - (now - _raw_proxy_evict_history[0]) + 0.999))
                return False, (
                    "SKIP_EVICT_GUARD: "
                    f"最近 {window_seconds:g} 秒已自动淘汰 {max_per_window} 条活动代理，"
                    f"暂停持久删除约 {retry_after} 秒；本轮仍会自动切换下一条。"
                )

            raw_conf["proxy_list"] = kept_source_entries
            raw_conf["success_proxy_list"] = kept_success_entries
            config_data["raw_proxy_pool"] = raw_conf
            runtime_cfg.reload_all_configs(new_config_dict=config_data)
            _raw_proxy_evict_history.append(now)
            if not kept_active_normalized:
                runtime_cfg.POOL_EXHAUSTED = True
            return True, (
                f"已从{active_pool_label}移除 {get_display_name(normalized_proxy)}"
                "，并同步清理原始池与成功池"
            )
...
def _do_smart_switch(proxy_url=None):
    ...
    try:
        ...
        desired_group_name = resolve_group_name(proxies_data, PROXY_GROUP_NAME)
        actual_group_name = resolve_switchable_group_name(proxies_data, PROXY_GROUP_NAME)
        ...
        if not actual_group_name:
            ...
            print(
                f"[{ts()}] [ERROR] {display_name} 找不到可切换的 Selector 策略组(配置='{PROXY_GROUP_NAME}')。"
                f" 当前可用策略组: {', '.join(clean_for_log(g) for g in available_groups[:8])}"
            )
            return False

        if desired_group_name and desired_group_name != actual_group_name:
            dtype = str((proxies_data.get(desired_group_name) or {}).get("type") or "")
            print(
                f"[{ts()}] [代理池] {display_name} 配置组 [{clean_for_log(desired_group_name)}] "
                f"类型={dtype or '?'} 不可 PUT 切换，自动改用 Selector [{clean_for_log(actual_group_name)}]"
            )
        elif actual_group_name and not is_switchable_group(proxies_data, actual_group_name):
            print(
                f"[{ts()}] [ERROR] {display_name} 策略组 [{clean_for_log(actual_group_name)}] 不是 Selector，无法切换。"
            )
            return False
        ...
        if FASTEST_MODE:
            ...
            try:
                delay_map = _fetch_group_delay_map(current_api_url, headers, actual_group_name)
                measured_nodes = [(node, delay_map[node]) for node in valid_nodes if delay_map.get(node, 0) > 0]
                nodes_with_delay = measured_nodes or nodes_with_delay
                best_node, min_delay = min(measured_nodes, key=lambda item: item[1]) if measured_nodes else (None, None)
                if best_node:
                    print(f"[{ts()}] [代理池] {display_name} 组测速完成，最快节点: [{clean_for_log(best_node)}] ({min_delay}ms)")
                    switch_resp = std_requests.put(
                        f"{current_api_url}/proxies/{safe_group_name}",
                        headers=headers, json={"name": best_node}, timeout=5
                    )
                    if switch_resp.status_code in {200, 204}:
                        time.sleep(1)
                        ...
                        if removed:
                            ...
                            print(f"[{ts()}] [代理池] {display_name} 测活失败，但已触发保底保护: {_format_skip_evict_guard_message(remove_msg)}")
                        else:
                    ...
                    print(f"[{ts()}] [代理池] {display_name} 组测速无可用结果，回退到随机抽卡模式...")
        ...
        for i in range(1, max_retries + 1):
            ...
            if switch_resp.status_code == 204:
                ...
                body = ""
                try:
                    body = (switch_resp.text or "")[:160]
                except Exception:
                    body = ""
                hint = ""
                if switch_resp.status_code == 400 and "Must be a Selector" in body:
                    hint = "（目标组不是 Selector，请把 clash 策略组改成 PROXY）"
                print(
                    f"[{ts()}] [代理池] {display_name} 指令下发失败 "
                    f"(HTTP {switch_resp.status_code} {body}){hint}"
                )
