...
async def git_update(req: GitSyncReq, token: str = Depends(verify_token)):
    ...
    if action not in {"fetch", "pull_ff", "reset_hard"}:
    ...
    tracking = str(before.get("tracking") or "")
    if action == "pull_ff":
        commands.append(["git", "pull", "--ff-only"])
    elif action == "reset_hard":
        commands.append(["git", "reset", "--hard", tracking or "origin/main"])
    ...
    for command in commands:
        ...
        if not result.get("ok"):
            last_error = result.get("error") or f"命令失败: {rendered}"
    ...
    message = {
        "fetch": "远端状态已刷新。",
        "pull_ff": "已完成 Git 快进更新。",
        "reset_hard": f"已强制同步到 {tracking or 'origin/main'}。",
